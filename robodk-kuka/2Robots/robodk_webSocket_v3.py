...
class WebSocketCommunication:
    def __init__(self, host, port, robot1, robot2, tool1, tool2, turntable1, turntable2, rdk):
        self.host = host
        self.port = port
        self.robot1 = robot1
        self.tool1 = tool1
        self.turntable1 = turntable1
        self.robot2 = robot2
        self.tool2 = tool2
        self.turntable2 = turntable2
        ...
        self.ik_thread1 = threading.Thread(target=self.ik_cal_thread1)
        ...
        self.ik_thread2 = threading.Thread(target=self.ik_cal_thread2)
        self.ik_thread2.start()
        self.reachable_event2 = asyncio.Event()

        print("webSocket Start!")

    def ik_cal_thread1(self):
        while True:
            if self.position1 and self.rotation1:
                # print("ik_cal Working!!")
                local_pose = KUKA_2_Pose(
                    [1000 * self.position1['x'], -1000 * self.position1['z'], 1000 * self.position1['y'],
                     self.rotation1['x'], self.rotation1['y'], self.rotation1['z']])
                all_solutions = self.robot1.SolveIK_All(local_pose, self.tool1)
                ...
                if len(all_solutions) == 0:
                    ...
                    for j in all_solutions:
                        conf_RLF = self.robot1.JointsConfig(j).list()
                        ...
                        if rear == 0 and lower == 0 and flip == 0:
                            self.ik_results1 = j
                            socket_lock.acquire()
                            self.robot1.setJoints(self.ik_results1[:6])
                            socket_lock.release()
                            # IK 계산이 완료된 후, position과 rotation을 None으로 설정
                            self.position1 = None
                            self.rotation1 = None
                            self.reachable1 = True
                            self.reachable_event1.set()  # IK가 가능할 때 이벤트 설정
                            print("IK reachable!!")
                            break
